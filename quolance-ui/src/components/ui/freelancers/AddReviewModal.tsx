...
interface AddReviewModalProps {
  ...
  errors: { title: boolean; comment: boolean };
  setErrors: React.Dispatch<
    SetStateAction<{ title: boolean; comment: boolean }>
  >;
  onSubmit: () => void;
}
...
function AddReviewModal({
  ...
  const closeModal = () => {
    setIsOpen(false);
    setStep(1);
    setErrors({ title: false, comment: false });
    setRatings({
      title: '',
        ...
        <div className='mx-3 my-20 w-[575px] rounded-2xl bg-white p-8 drop-shadow-2xl'>
          ...
          <div className='flex items-center justify-between gap-3'>
            <div className='flex items-center gap-3'>
              <PiStar className='text-xl text-yellow-500 sm:text-3xl' />
            ...
            <div className='py-6'>
              ...
              <div className='mt-4 flex flex-wrap justify-between'>
                {ratingOptions.map((option) => (
                  <button
                    key={option.label}
                    className={`mt-2 flex w-full flex-col items-center rounded-lg border px-3 py-2 sm:w-24 ${
                      ratings.qualityOfDeliveryRating === option.value
                        ? 'border-black'
                        : 'border-gray-300'
                    }`}
                    onClick={() =>
              ...
              <div className='mt-4 flex flex-wrap justify-between'>
                {ratingOptions.map((option) => (
                  <button
                    key={option.label}
                    className={`mt-2 flex w-full flex-col items-center rounded-lg border px-3 py-2 sm:w-24 ${
                      ratings.communicationRating === option.value
                        ? 'border-black'
                        : 'border-gray-300'
                    }`}
                    onClick={() =>
                      handleRatingChange('communicationRating', option.value)
                    }
                  >
                    <span className='text-2xl sm:text-lg'>{option.emoji}</span>
                    <span className='text-md  sm:text-xs'>{option.label}</span>
              ...
              <div className='mt-4 flex flex-wrap justify-between'>
                {ratingOptions.map((option) => (
                  <button
                    key={option.label}
                    className={`mt-2 flex w-full flex-col items-center rounded-lg border px-3 py-2 sm:w-24 ${
                      ratings.qualityOfWorkRating === option.value
                        ? 'border-black'
                        : 'border-gray-300'
                    }`}
                    onClick={() =>
                      handleRatingChange('qualityOfWorkRating', option.value)
                    }
                  >
                    <span className='text-2xl sm:text-lg'>{option.emoji}</span>
                    <span className='text-md  sm:text-xs'>{option.label}</span>
          ...
          {step === 4 && (
            <div className='py-6'>
              <p className='text-n300 text-md mb-4'>
          ...
          >
            {step > 1 && (
              <button
                ...
                className='bg-n30 hover:text-n900 relative flex items-center justify-center overflow-hidden rounded-full px-6 py-2 font-medium duration-700 after:absolute after:inset-0 after:left-0 after:w-0 after:rounded-full after:bg-yellow-400 after:duration-700 hover:after:w-[calc(100%+2px)]'
              >
