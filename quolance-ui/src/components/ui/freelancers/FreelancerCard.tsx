...
interface FreelancerCardProps {
  ...
  status: ApplicationStatus; // Add this
}
...
function FreelancerCard({
  ...
  return (
    <>
      <div
        className={`border-n40 bg-n10 relative flex min-w-[414px] max-w-[630px] flex-1 flex-col gap-6 rounded-3xl border py-6 transition-all duration-200 ${
          selected ? 'ring-2 ring-blue-500 ring-offset-2' : ''
        }`}
      >
        <div className='absolute right-7 top-7 z-30 flex flex-row-reverse items-center gap-4'>
          <label className='inline-flex cursor-pointer items-center'>
            ...
            />
            <span className='peer relative h-6 w-6 rounded border-2 border-gray-300 bg-white after:absolute after:left-1/2 after:top-1/2 after:h-3 after:w-3 after:-translate-x-1/2 after:-translate-y-1/2 after:bg-blue-500 after:opacity-0 after:transition-opacity peer-checked:border-blue-500 peer-checked:after:opacity-100'></span>
          </label>
        ...
        <div className='flex items-center justify-start gap-2 px-6'>
          <button
            ...
            className='bg-n700 hover:text-n900 relative w-full overflow-hidden rounded-full px-6 py-[10px] text-sm font-semibold text-white duration-700 after:absolute after:inset-0 after:left-0 after:w-0 after:rounded-full after:bg-yellow-400 after:duration-700 hover:after:w-[calc(100%+2px)]'
          >
