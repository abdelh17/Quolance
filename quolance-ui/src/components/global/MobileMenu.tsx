  ...
  return (
    <nav className=''>
      <div
        ...
        } black/50 shadow-lg fixed top-0 z-[998] h-full w-full duration-700 4xl:hidden ltr:left-0 rtl:right-0`}
        onClick={() => setShowMobileMenu(false)}
      ></div>

      <div
        className={`bg-b300 fixed top-0 z-[999] flex h-full w-3/4 flex-col items-start justify-start gap-8 text-white/80 duration-700 min-[500px]:w-1/2 4xl:hidden lg:gap-20 ltr:left-0 rtl:right-0 ${
          showMobileMenu
            ? 'visible translate-y-0 opacity-100  delay-500'
            : ' invisible translate-y-[100%] opacity-50'
        }`}
      >
        <div className='bg-white fixed top-0 flex w-full items-center justify-between p-4 sm:p-8 mt-2 mb-6'>
        ...
        <ul className='flex w-full flex-col items-start gap-6 overflow-y-auto pb-10 pt-24 text-lg sm:text-xl lg:gap-10 ltr:pl-8 rtl:pr-8'>
          {headerMenu.map((menu) => (
            <li key={menu.id}>
              {menu.isSubmenu ? (
                <div className='subMenuToggle group flex flex-col items-start justify-start mt-6'>
                  <div
        ...
        <Link
          href=''
          className='mb-6 bg-y300 text-n900 rounded-full px-4 py-2 text-base font-medium ltr:ml-4 rtl:mr-4'
        >
          Offer Your Services
        </Link>
      </div>
